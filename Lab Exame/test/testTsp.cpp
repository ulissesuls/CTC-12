...
void testTspFile(std::string filename, int TspSize, long TspCost) {
    ...
    float approx_factor = 2;
#ifdef CTC12_CHRISTOFIDES
    approx_factor = 1.51;
#endif
    std::cout << "\n FILE: "<<  filename << "   cost: " << your_perccost << " approx_factor = " << approx_factor << "\n"; 
    
    ASSERT_LE(your_perccost,ceil(approx_factor*TspCost));
    delete ts;
}//testTspFile
...
/// ********** corectness test  ********************
TEST_F(TspTest, WestSahara29Tsp) {
    testTspFile("../data/wi29.tsp",29,27603);
}
